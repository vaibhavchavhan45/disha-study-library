...
const AddWaitingStudentModal = ({ onClose, onSuccess }) => {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch(urlConfig.cloudinaryUploadUrl, { method: "POST", body: data });
  ...
  return (
    <Modal title="Add Student" subtitle="Add a new student to the waiting list" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          <input type="text" name="name" placeholder="Student full name" value={form.name} onChange={handle}
            className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-gray-400 transition" />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="text" name="email" autoComplete="off" placeholder="example@gmail.com" value={form.email} onChange={handle}
              className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-gray-400 transition" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Fee Status</label>
        ...
        {/* Pending Amount — full width, only when PENDING */}
        {form.fee_status === "PENDING" && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Pending Amount (₹)</label>
            <input type="number" name="pending_amount" placeholder="Enter unpaid amount" value={form.pending_amount} onChange={handle}
              className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-gray-400 transition" />
          </div>
        )}
